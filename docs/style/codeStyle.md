...
Startdis的编码规范遵从于《阿里巴巴JAVA开发规约》和社区制定的Startdis代码风格文件。
...
## Startdis Code Style File

### Idea IDE

Startdis Code Style file is `style/startdis-code-style-for-idea.xml` in source code. Developers can import it to Idea IDE and reformat code by IDE.

Startdis代码风格文件在源代码下的`style/startdis-code-style-for-idea.xml`文件中，开发者可以将其导入到Idea IDE中，并让IDE帮助您格式化代码。
...
3. Import `style/StartdisCheckStyle.xml` to checkstyle plugin.
...
3. 导入源代码下`style/StartdisCheckStyle.xml`文件到checkstyle插件。
